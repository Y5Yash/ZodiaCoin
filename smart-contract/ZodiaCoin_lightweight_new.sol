...
contract ZodiaCoin is ERC20 {
    ...
    function mint(uint32 _epoch, uint256 _day, uint256 _month, uint256 _year, string memory _provider, address _contextAddress, ReclaimContractInterface.CompleteClaimData memory _claimData, bytes[] memory _signatures) public {
        ...
        require( keccak256(abi.encodePacked(_provider)) == keccak256(abi.encodePacked(provider)) );
